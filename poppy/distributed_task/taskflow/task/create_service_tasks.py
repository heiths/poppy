...
class CreateProviderServicesTask(task.Task):
    """Create external services on CDN providers."""
    default_provides = "responders"

    def execute(self, providers_list_json, project_id, service_id):
        """Attempts to create services for each :term:`provider` in ``providers_list_json``.
        ...
        :param  providers_list_json: A serialized list of providers.  i.e. ``u'["akamai"]'``
        :type providers_list_json: unicode
        :param service_id: A generated :py:func:`uuid.uuid4` that represents a service
        :type service_id: unicode
        :param project_id: A project/tenant id
        :type service_id: unicode

        :return: List of results from :py:meth:`poppy.provider.base.responder.Responder.created`
        :rtype: list[dict]
        :raises ValueError: If no service exists with the given ``project_id``.
        """
...
class CreateLogDeliveryContainerTask(task.Task):
    ...
    def execute(self, project_id, auth_token, service_id):
        """Create an object storage container for logs (if enabled).
